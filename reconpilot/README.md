...
## Why this design wins the judging themes

| Theme | Where it shows |
|---|---|
| Human-in-the-loop | Agent can only draft; approve/reject is a supervisor action; overrides journaled |
| Explainability | Reasoning trace per break + append-only audit log + evidence-grounded narrative |
| ₹-quantified ROI | TAT penalty clock (₹100/day/break), FTE math on every dashboard load |
| Integration realism | Consumes the same end-of-cycle file drops a Finacle bank already produces — day-1, zero core surgery; adapters (`app/recon/normalize.py`) are the only code that changes when IDBI's sandbox APIs replace the mocks |
| Regulatory fluency | RBI TAT circular, URCS/UDIR vocabulary (TCC/RET), suspense GLs, deemed transactions |
